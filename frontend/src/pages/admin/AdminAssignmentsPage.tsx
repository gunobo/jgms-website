...
export function AdminAssignmentsPage() {
  ...
  const load = () =>
    api
      .get<AssignmentListItem[]>("/api/admin/assignments")
      .then(setAssignments)
  ...
  const togglePublish = async (a: AssignmentListItem) => {
    await api.post(`/api/admin/assignments/${a.id}/publish`, undefined, { published: !a.is_published });
    load();
  };

  const remove = async (id: string) => {
    if (!confirm("이 과제를 삭제할까요? 제출물과 채점 데이터도 함께 삭제됩니다.")) return;
    await api.del(`/api/admin/assignments/${id}`);
    load();
  };
